...
export default (nodeName: string, node: any, createEntity: Function): EntityInstance | null => {
  ...
  const isLegacyImage = nodeName === 'img' && isNotAtomicBlockNode(node.parentNode) && isNotAtomicBlockNode(node.parentNode.parentNode);
  ...
  const isImage = isAtomicBlock && node.firstChild && node.firstChild.dataset && node.firstChild.dataset.mimetype && node.firstChild.dataset.mimetype.startsWith('image');
  if (isImage) {
    return createEntity(ENTITY_TYPES.image, ENTITY_MUTABILITY.immutable, {
